  ...
  inbound {
    ...
    ipv4     = concat(var.settings.server.allowedIps.ipv4,
                      [
                        ...
                        "${linode_instance.codeQuality.ip_address}/32",
                        "${linode_instance.codeQuality.private_ip_address}/32",
...
# Firewall rules to protect Sonarqube server.
resource "linode_firewall" "codeQuality" {
  label           = "${var.settings.codeQuality.name}-fw"
  ...
  inbound {
    ...
    protocol = "TCP"
    ports    = "22,2222,80,443"
    ipv4     = concat(var.settings.codeQuality.allowedIps.ipv4,
      [
        "${linode_instance.server.ip_address}/32",
        "${linode_instance.server.private_ip_address}/32",
        "${linode_instance.runner.ip_address}/32",
    ...
    ipv6     = concat(var.settings.codeQuality.allowedIps.ipv6, [ "::1/128" ])
  }
  ...
  linodes = [ linode_instance.codeQuality.id ]
